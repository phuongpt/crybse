...
/// @nodoc
mixin _$OrderBook {
  List<OrderPrice> get asks => throw _privateConstructorUsedError;
  List<OrderPrice> get bids => throw _privateConstructorUsedError;
...
abstract class $OrderBookCopyWith<$Res> {
  ...
  @useResult
  $Res call({List<OrderPrice> asks, List<OrderPrice> bids, int seqNum});
}
...
class _$_OrderBook implements _OrderBook {
  const _$_OrderBook(
      final List<OrderPrice> asks, final List<OrderPrice> bids, this.seqNum)
      : _asks = asks,
        _bids = bids;

  final List<OrderPrice> _asks;
  @override
  List<OrderPrice> get asks {
    if (_asks is EqualUnmodifiableListView) return _asks;
    // ignore: implicit_dynamic_type
    return EqualUnmodifiableListView(_asks);
  }

  final List<OrderPrice> _bids;
  @override
  List<OrderPrice> get bids {
...
abstract class _OrderBook implements OrderBook {
  const factory _OrderBook(final List<OrderPrice> asks,
      final List<OrderPrice> bids, final int seqNum) = _$_OrderBook;
